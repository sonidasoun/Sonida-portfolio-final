            ...
            <ul class="grid gap-4 mt-8 sm:grid-cols-2 lg:grid-cols-4">
                <li v-for="(project, index) in projects" :key="index">
                    <router-link :to="`project/${project.name}`"
                        class="block p-4 border border-gray-300 rounded-md shadow-sm sm:p-6">
                        <div class="sm:flex sm:justify-between sm:gap-4 lg:gap-6">
                            ...
                            <div class="mt-4 sm:mt-0">
                                <h3 class="text-lg font-medium text-gray-900 text-pretty">
                                    {{ project.name }}
                                </h3>

                                <p class="mt-1 text-sm text-gray-700">By John Doe</p>

                                <p class="mt-4 text-sm text-gray-700 line-clamp-2 text-pretty">
                                    {{ project.description }}
                                </p>
                            </div>
                        </div>

                        <dl class="flex gap-4 mt-6 lg:gap-6">
